...
public class CreatePlayer : MonoBehaviour {
	...
	void Update () {
		if(!player1IsActive && Input.GetButtonDown("Fire1P1")){;
			...
			PlayerScript p1 = ((GameObject)Instantiate(player, new Vector3(-6f, -3f, 0), Quaternion.identity)).GetComponent<PlayerScript>();
			p1.myPlayer = 1;
		}
		if(!player2IsActive && Input.GetButtonDown("Fire1P2")){
			Debug.Log ("I am player 2");
			player2IsActive = true;
			PlayerScript p1 = ((GameObject)Instantiate(player, new Vector3(-3f, -3f, 0), Quaternion.identity)).GetComponent<PlayerScript>();
			p1.myPlayer = 2;
		}
		if(!player3IsActive && Input.GetButtonDown("Fire1P3")){
			Debug.Log ("I made a thing");
			player3IsActive = true;
			PlayerScript p1 = ((GameObject)Instantiate(player, new Vector3(-0f, -3f, 0), Quaternion.identity)).GetComponent<PlayerScript>();
			p1.myPlayer = 3;
		}
